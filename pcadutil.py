...
def pxagecritica(odtoprm):
    ...
    for col in range(0, 15, 3):
        ...
        if odtoprm.values[coluna1] is True and \
            (odtoprm.values[coluna2] is False and odtoprm.values[coluna3] is False):
            sg.popup_ok('Informe se o atendimento é de Manhã ou a \
                Tarde para {}'.format((tabdia[cold])))
            wret = False
            break
...
def pxagemntsqlalt(odtoprm):
    ...
    for col in range(0, 15):
        ...
        if col < 14:
            campo += '     {} = {}, \n'.format(wag, (dcfv[str(odtoprm.values[wcb])]))
        else:
            campo += '     {} = {} \n'.format(wag, (dcfv[str(odtoprm.values[wcb])]))
...
def pxxcriakey():
    ...
    lsdataKey = lsdatafrmt[6:8]+lsdatafrmt[3:5]+lsdatafrmt[0:2] + \
        lsdatafrmt[9:11]+lsdatafrmt[12:14]+lsdatafrmt[15:17] + \
        lsdatafrmt[18:20]
...
def pxxobtermes(pwsdata):
    ...
    """
    #
    meses = ('mes', 'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho',
