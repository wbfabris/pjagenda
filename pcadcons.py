...
def pxtelacria(odtoprm):
    ...
    # --- Cabeçalho ----
    headings = ['Nome', 'Local', 'Endereço', 'Nº', 'Comp', 'Tel1', 'Agenda', 'Conv', 'Cel1']

    # --- Tamanho dos Campos do Cabeçalho ----
    data_cols_width =  [20, 9, 20, 5, 5, 15, 34, 15, 15,]

    # --- Layout da Tela ----
    layout = [[sg.Table(values=odtoprm.datafrmt,
                headings=headings,
                ...
                num_rows=30,justification='left', #41
                enable_events=True, key='_filestable')],
...
def pxproctela(odtoprm):
    ...
    # --- Cria a Tela ----
    window = sg.Window('Relação dos medicos', odtoprm.layout, location=(0, 0), finalize=True)
    window.maximize()
