...
def pxprofinc(odtoprm):
    ...
    wsql = wsql + 'VALUES("{}", "{}")'.format(odtoprm.values['nome'], odtoprm.values['crm'])

    odb = Db(odtoprm.pathdb, wsql, '', '')
    odb.db_conecta()
    valoretorno =  odb.db_executa()
    if valoretorno is None:
        wsql = 'SELECT {} FROM {} WHERE {} = {}'. format('id_med', wtab, \
         'nm_med', chr(34) + odtoprm.values['nome'] + chr(34))
        odb.db_sql = wsql
...
def pxprofalt(odtoprm):
    ...
    wsql = wsql + ' SET nm_med = ' + chr(34) + odtoprm.values['nome'] + chr(34) + ',\n'
    wsql = wsql + '     nu_crm = ' + chr(34) + odtoprm.values['crm'] + chr(34)  + '\n'
    wsql = wsql + ' {} = {}'.format('WHERE id_med ', odtoprm.data[odtoprm.row][35])
...
def pxprofexc(odtoprm):
    ...
    valoretorno = sg.popup_ok_cancel('Exclusão,\
     Confirma a Exclusão de {' + odtoprm.values['nome'] + '}?')
    if valoretorno == 'OK':
